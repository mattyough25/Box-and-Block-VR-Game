...
sFile = sprintf('OCULUS_Test_BB_VR_2_%s.bin', datestr(now,'mm-dd-yyyy-HH-MM')); %Automated naming of Oculus
sFileLocation = ['ObjectLocationData_Set_01.csv']; % table, drop target, and block location file

%% Define Table Spawn Location
nPawnLocation = [-541 100 -85];
...
nBlock_Location_Left = [...
    nTableLocation(1)+15   nTableLocation(2)-13 nTableLocation(3)-2;...
    nTableLocation(1)+15   nTableLocation(2)-13 nTableLocation(3)-2;...
    nTableLocation(1)+15   nTableLocation(2)-13 nTableLocation(3)-2;...
    nTableLocation(1)+15   nTableLocation(2)-13 nTableLocation(3)-2;...
    nTableLocation(1)+15   nTableLocation(2)-13 nTableLocation(3)-2;...
...
nDrop_Target_Location = [nTableLocation(1)+14 nTableLocation(2)+14 nTableLocation(3)-3.6; nTableLocation(1)+14 nTableLocation(2)-13 nTableLocation(3)-3.6];
...
nTime.StartTime = tStartTime.data.t;
...
for iRep = 1:nRep
    ...
    nTime.SpawnTime(iRep) = tSpawnTime.data.t;

    if iRep == 1
        [nBlockDropTime(iRep),tCountTime] = wait_for_block_drop(par);
    else
        [nBlockDropTime(iRep),tCountTime2] = wait_for_block_drop(par);
